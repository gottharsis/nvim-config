...
function M.set_keymaps(client, bufnr)
    ...
    -- vim.keymap.set("i", "<C-k>", vim.lsp.buf.signature_help, { desc="Method signature", buffer=bufnr})
    vim.keymap.set("n", "<leader>wa", vim.lsp.buf.add_workspace_folder, { desc="Add workspace folder", buffer=bufnr})

    vim.keymap.set("n", "<leader>wr", vim.lsp.buf.remove_workspace_folder, { desc="Remove workspace folder", buffer=bufnr})
    vim.keymap.set("n", "<leader>wl", function () print(vim.inspect(vim.lsp.buf.list_workspace_folders())) end, { desc="List workspace folders", buffer=bufnr})
    ...
    vim.keymap.set("n",  "<leader>m", ":SymbolsOutline<cr>", {desc="Toggle Symbols Outline" })

    vim.keymap.set("n", "<leader>s", require("telescope.builtin").lsp_document_symbols, { desc="Document Symbols", buffer=bufnr})

    vim.keymap.set("n", "<leader>S", require("telescope.builtin").lsp_dynamic_workspace_symbols, { desc="Workspace Symbols", buffer=bufnr})
